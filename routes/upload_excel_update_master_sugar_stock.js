...
router.post('/', async function(req, res, next)
{
 ...
 form.parse(req, function (err, fields, files) {
  ...
  fs.rename(oldpath, 'uploaded_files/sample_update_master_sugar_stock.xlsx'  /*newpath*/, function (err) {
    if (err) 
    {
...
*/
...
else
    {
       ...
       wb.xlsx.readFile('uploaded_files/sample_update_master_sugar_stock.xlsx').then(function(){
       wb.csv.writeFile('uploaded_files/sample_update_master_sugar_stock.csv')
       .then(async function() {
       console.log("saved csv...done");
       var line_cnt=0;
       //sql = "delete from master_sugar_stock";
       //console.log(sql);
       //await db.query(sql);
       lineReader.eachLine('uploaded_files/sample_update_master_sugar_stock.csv', async function(line, last) {
       line_cnt++;
       if (line_cnt >= 17)
       {
        var arr = line.split(",");
